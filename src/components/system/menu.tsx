    ...
    <div ref={boxRef} className={cn("relative inline-block", className)} onKeyDown={onKeyDown}>
      ...
      {open && (
        <div
          ...
          aria-label={label}
          className={cn(
            "absolute z-30 mt-2 min-w-[200px] overflow-hidden rounded-xl border border-border bg-white py-1 shadow-xl",
            align === "end" ? "right-0" : "left-0",
          )}
        >
          {items.map((item, i) => {
            const cls = cn(
              "block w-full px-4 py-2.5 text-left text-[15px] transition-colors duration-200",
