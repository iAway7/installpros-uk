...
/**
 * Segments offered, one tab each on the /faqs hub.
 *
 * These used to be service lines (Home Network, VoIP, Digital Signage and so
 * on) and seven of the eight tabs were empty: every question in the file was a
 * Starlink one. The axis is now the install type, which is what the funnel
 * actually sells and what the lead form already asks for.
 */
...
/** A question and its answer. Which segment it belongs to comes from the
 *  array it lives in, not from a field, so nothing can contradict itself. */
...
/** A FaqEntry once it has been placed in a segment, for the /faqs hub. */
 ...
 * Abound, not from us.
 */
export const COMMERCIAL_FAQS: FaqEntry[] = [
  {
    category: "Pricing",
 ...
 * what is said.
 */
export const VEHICLE_FAQS: FaqEntry[] = [
...
/**
 * The list behind the /faqs hub. Built from the arrays above rather than
 * copied, so every answer has one home: fix it once and the landing and the
 * hub both change.
 *
 * Maritime has no questions yet and is deliberately absent. FaqTabs renders
 * its tab with the "coming soon" state until someone writes them, and they
...
export const ALL_FAQS: Faq[] = [
  ...
  ...VEHICLE_FAQS.map((f) => ({ ...f, service: "Mobile/RV" as const })),
];
